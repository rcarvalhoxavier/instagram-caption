...
test("JSON variant decodes double-escaped unicode", () => {
  ...
  // embeds it. A single decode leaves the raw surrogate-pair escape text
  // literal in the title, instead of the decoded rocket this test requires.
  const result = classify(load("embed_json"));
...
test("no author and no broken marker means unknown", () => {
  // Central invariant: a block must NEVER become gone, because a caller may
  // turn gone into a permanent decision it never revisits.
...
test("unescapeHtml maps each entity to the right character", () => {
  // unescapeHtml is internal to the package but exported from this module, so
  // its contract still stands on its own: classify happens to fold whitespace
  // afterwards, which masked a wrong &nbsp; once.
...
test("an author marker that trims to nothing is unknown, not found", () => {
  ...
  // whitespace only. Letting it through yields the author "", which callers
  // would render as "@" -- worse than admitting we do not know.
...
test("an out-of-range numeric entity is left alone, never thrown on", () => {
  ...
  // that was only asking about one post.
